...
/**
 * Servlet implementation class ServletAR
 */
@WebServlet(name="Añadir usuario", urlPatterns="/ServletAR")
public class ServletAR extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(usuarioBean!=null){
			...
			if(!recurso.isBlank() && !curso.isBlank()){
				...
				if(cursoDAO.checkNombreCurso(curso, properties, credentials)) {
					...
					if(cursoDAO.checkcoordinadorrecursos(id_usuario,id_curso, properties, credentials))	{
						cursoDAO.addRecurso(id_curso,recurso, properties, credentials);
					}else {
